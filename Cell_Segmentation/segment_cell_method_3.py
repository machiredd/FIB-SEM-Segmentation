...
mask_nums_1 = [num + 1 for num in mask_nums]

for num in mask_nums_1[1:]:
    for i in range(num, num - 50, -1):
        ...
        # Read the mask and border images
        ...
        # Compute optical_flow from previous image and propogate the 
        # boundary estimated for the previous image using mask and optical flow information
        # instead of propogating boundary directly from ground truth image
    
        if image_num - 1 in mask_nums:
            image_no = f'{image_num - 1:03d}'
            name = os.path.join(gt_path, f'SMMART_SL2_101a_cell_contours_crop_{image_no}.tif')
            currentimage = cv2.imread(name, cv2.IMREAD_GRAYSCALE)
            of_image = currentimage
            of_image[of_image > 0] = 255
        else:
            of_image = compute_of(image_num, new_boundary, img_direc)
        ...
        ## Mask + boundary + Optical flow (full) 
    
        mask2[of_image > 0] = 0
        seg_mask = mask2
    
        ## Mask + boundary + Selected Optical flow (only at overlapping cells)
